...
namespace HotelClientMVC.Controllers
{
    ...
    {
        string baseUrl = "https://localhost:7169/api/Auth/";
        string baseUrlRegister = "https://localhost:7169/api/User/register";
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            try
            {
                ...
                var response = await _httpClient.PostAsync($"{baseUrl}login", content);//baseUrl calisacaqmi baxacam
        ...
        public async Task<IActionResult> Register(RegisterViewModel model)
        {
            ...
            var content = new StringContent(JsonConvert.SerializeObject(model), Encoding.UTF8, "application/json");
            var response = await _httpClient.PostAsync(baseUrlRegister, content);
            if (response.IsSuccessStatusCode)
            {
